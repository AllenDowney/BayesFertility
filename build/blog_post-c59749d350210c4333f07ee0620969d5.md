...
where $\lambda_{ij}$ is the expected birth rate for cohort $i$ at age $j$. The parameters are on a log scale, so they're not easy to interpret in absolute terms. Instead, we should think of them as differences in fertility between groups.
...
We can validate the model on cohorts who have already completed their childbearing. The first figure above shows the model's predictions and observed completed fertility from Census data. The agreement is very good.
...
The extended model adds a timing shift parameter (γ):

$$\log(\lambda_{ij}) = \alpha_i + \beta_j + \gamma_i \times \text{age}_j$$

where $\gamma_i$ is a cohort-specific parameter that shifts the age-fertility curve. Positive values of $\gamma$ mean delayed childbearing (fertility shifts to older ages), while negative values mean earlier childbearing (fertility shifts to younger ages).
